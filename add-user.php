...
<div class="wrapper" id="myDiv">
  ...
  <div class="content-wrapper">
    ...
    <?php
              ...
              $fetch_user = "SELECT`u_name`, `emp_id` FROM `tbl_users` INNER JOIN employess ON tbl_users.emp_id = employess.id WHERE u_id = '$user_get_id' ";
    $run_user = mysqli_query($conn, $fetch_user);
    $row_user = mysqli_fetch_array($run_user);
            $user_name = $row_user['u_name'];
            $user_pass = $row_user['emp_id'];
 ...
 if(isset($_GET['userID']))
                { ?>
                     <input type="hidden"  name="emp_ids" class="form-control" id="i" value="<?php if(isset($user_pass)){echo $user_pass; } else { echo ""; } ?>" >
<input type="text" name="user" class="form-control" id="i" value="<?php if(isset($user_name)){echo $user_name; } else { echo ""; } ?>" >
  <?php
}
      else {          
    ?>
                    <select class="form-control" name="user" id="usersa" required>
                        <option value=""></option>
                        ...
                        while($data = mysqli_fetch_array($role_result)){
                            ...
                            ?>
                                <option value="<?php echo $data['id'].",".$data['First_Name']; ?>">
                  ...
                  <div class="form-group">
                    ...
                    <input type="password"  name="password" class="form-control" id="password-field" placeholder="Enter Password" value=""  required>
...
<?php
               ...
               if(isset($_POST['reg'])){
                  ...
                  // $role = $_POST['role'];
                   ...
                   $new_data = explode(',', $user);
$emp_id = trim($new_data[0]);
$emp_name = trim($new_data[1]);
                   
                   $sql = "INSERT INTO `tbl_users`(`u_name`, `u_pass`,`emp_id`, `role_id`, `u_status`, `date_time`) VALUES ('$emp_name','$password','$emp_id','5','active', current_timestamp() )";
                  ...
                  echo  $update_sql = "UPDATE `tbl_users` SET `u_name`='$username',`u_pass`='$password',`emp_id`='$empids',`u_status`='active' WHERE `u_id` = '". $_GET['userID'] ."' ";
...
$users_query = "SELECT u_id, u_name, u_pass, u_status FROM `tbl_users` WHERE u_status = 'active' ";      
